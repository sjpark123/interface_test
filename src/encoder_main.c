...
#endif
{
	...
	printf("Display Stream Compression (DSC) Encoder version 1.48\n");
	printf("Copyright 2013-2015 Broadcom Corporation.  All rights reserved.\n\n");
	...
	{
		fprintf(stderr, "4:2:2/4:2:0 not supported with RGB input\n");
		exit(1);
	}
	...
	while ((strlen(infname)>0) || !feof(list_fp) || multiFrameYuvFile)
	{
		...
		dsc_codec.dsc_version_minor = dscVersionMinor;
			...
			if(enableVbr)
			{
				...
				RANGE_CHECK("bits_per_pixel (*16)", target_bpp_x16, 96, 832);  // Top is 52bpp, bit rate for force_mpp with 16bpc
			}
			...
			dsc_codec.second_line_bpg_ofs = secondLineBpgOfs;
			RANGE_CHECK("second_line_bpg_offset", dsc_codec.second_line_bpg_ofs, 0, 31);
			dsc_codec.xstart = 0;
			dsc_codec.ystart = 0;
			...
			if (!sliceHeight)  // Auto-detect optimal size height
			{
				while(sliceh)
				{
					...
					{
						printf("PPS was not valid for slice height = %d, trying slice_height = %d\n", sliceh, sliceh>>1);
						sliceh >>= 1;
						dsc_codec.slice_height >>= 1;
					} else
						break;
				}
				if (!sliceh)
			...
			dsc_codec.initial_offset += secondLineOfsAdj;
		...
		if(dsc_codec.native_420)
			op_dsc = (pic_t *)pcreate(FRAME, ip->color, YUV_444, dsc_codec.pic_width/2, dsc_codec.pic_height);
		else if(dsc_codec.native_422)
			op_dsc = (pic_t *)pcreate(FRAME, dsc_codec.convert_rgb ? RGB : ip->color, YUV_422, dsc_codec.pic_width, dsc_codec.pic_height);
		else
			op_dsc = (pic_t *)pcreate(FRAME, dsc_codec.convert_rgb ? RGB : ip->color, YUV_444, dsc_codec.pic_width, dsc_codec.pic_height);
		...
		for (ys = 0; ys < dsc_codec.pic_height; ys+=sliceh)
		{
			...
			// Write compressed bitstream data
        ...
        //SBR:OPTION 5
		if (dsc_codec.native_420) {
			ip2 = pcreate(FRAME, ref_pic->color, YUV_420, op_dsc->w*2, op_dsc->h);
		...
		// Write reference files and compute PSNR
		if (ip)
		{
			if (yuvFileOutput)
			{
